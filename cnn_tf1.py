...
with graph.as_default():
    '(batch, 128,47) --> (batch, 64, 18)'    
    conv1 = tf.layers.conv1d(inputs = inputs_, filters=18, kernel_size=2, strides=1, padding='same', activation=tf.nn.relu, name='Convolution_Layer_1')
    max_pool_1 = tf.layers.max_pooling1d(inputs=conv1, pool_size=2, strides=2, padding='same', name='max_pooling_1')
...
#        x_b, y_b = tf.train.batch( [X_tr,y_tr], batch_size = 100 )
        ...
        for x,y in get_batches(X_tr, y_tr, batch_size):    
            ...
            x = x.reshape(-1,128,47)
            
            #y = np.argmax(y, axis = 1)
            yy = []
            for i in range(640//128):
                offset = (i * 128) % (640 - 128)
                y1 = y[offset:(offset + 128), :]
#                y = (y[i: i + i*128,])
...
#                y = y.reshape(-1,10)
            ...
            if (iteration%10 == 0):                
                ...
                for x_v, y_v in get_batches(X_vld, y_vld, batch_size):
                    ...
                    x_v = x_v.reshape(-1,128,47)
            
                    #y = np.argmax(y, axis = 1)
                    yy_v = []
                    for i in range(640//128):
                        offset = (i * 128) % (640 - 128)
                        y1_v = y_v[offset:(offset + 128), :]
        #                y = (y[i: i + i*128,])
...
with tf.Session(graph=graph) as sess:
    ...
    for x_t, y_t in get_batches(X_test, y_test, batch_size):
        ...
        x_t = x_t.reshape(-1,128,47)
    
        #y = np.argmax(y, axis = 1)
        yy_t = []
        for i in range(640//128):
            offset = (i * 128) % (640 - 128)
            y1_t = y_t[offset:(offset + 128), :]
    #                y = (y[i: i + i*128,])
